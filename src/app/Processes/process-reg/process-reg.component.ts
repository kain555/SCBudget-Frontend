...
@Component({
  selector: 'app-process-reg',
  templateUrl: './process-reg.component.html',
  styleUrls: ['./process-reg.component.css']
})
export class ProcessRegComponent implements OnInit {
  ...
  ngOnInit() {
    this.httpService.getDirectorsReg().subscribe(
      emp => {
        this.employees = emp;
      }
    );
  ...
  showBudgets() {
    ...
    this.httpService.GetPentingDirectorReg(this.reqID).subscribe(
      (pDirector: []) => {
        this.cars = pDirector;
  ...
  accept(process_id: number) {
    let asignedTo = this.asignedTo[Math.floor(Math.random() * this.asignedTo.length)];
